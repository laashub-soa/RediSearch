...
IndexIterator *createNumericSkiplistUnionIterator(const IndexSpec *sp, NumericSkiplist *t,
  ...
  skiplistIterator *iter = slIteratorCreate(t->sl, (void *)&f->min);
  if (!iter) return NULL;
  ...
  if (vectorLen == 1) {
    ...
    Vector_Get(v, 0, &nsn);
    IndexReader *ir = NewNumericSkiplistReader(sp, nsn);
...
struct indexIterator *NewNumericIterator(RedisSearchCtx *ctx, const NumericFilter *flt,
...
NumericSkiplistIterator *NumericSkiplistIterator_New(NumericSkiplist *t) {
  return slIteratorCreate(t->sl, NULL);
}
